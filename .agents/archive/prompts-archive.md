...
## prompts.md retired 2026-07-06

The live queue file (.agents/prompts.md) is discontinued as of this date -
see AGENTS.md and conventions.md for the current process. It was designed
for asynchronous handoff (an agent autonomously discovers and pulls pending
work from the file); in practice, task specs have been handed to agents
synchronously instead, so the capability this file existed for was not
being exercised. All 5 entries still present in the live file at the time
of retirement are preserved below, status corrected where it had gone
stale relative to actual JIRA/repo state.

### PROMPT-021
Agent: Frontend Dev
JIRA: RF-29
Status: Pending (still true - RF-29 remains blocked on RF-58, see JIRA)
...
JIRA: RF-30
Status: Pending (still true - RF-30 remains blocked on RF-47, see JIRA)
...
Agent: Quality Analyst
JIRA: RF-31
Status: Pending (still true - RF-31 remains blocked on RF-47, see JIRA)
...
JIRA: RF-32
Status: Done (was still marked Pending in the live file - stale bookkeeping; RF-32 actually completed earlier in Sprint 2)
...
Agent: CyberSecurity Eng
JIRA: RF-33
Status: Done (was still marked Pending in the live file - stale bookkeeping; actually completed 2026-07-05 via PR #47, see progress.md)
