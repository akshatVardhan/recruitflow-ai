...
- If blocked on a missing API: check the API documentation, then add JIRA comment
...
12. Pre-commit checks mandatory: run `npm run build`, `npx eslint .`, and `npx prettier --check .` from the /frontend directory before every commit; all must pass with zero errors; never use --no-verify to bypass
